...
## TO NOTE: Multuple files exist in the SmallSampleFiles directory
import os

## Overwriting the deIDItem class to include the tileSource
...
class DeIdImageItem:
    def __init__(self, filePath=None, metadata=None):
        ...
        self.meta = {"largeImage": {"fileId": "TBD"}, "updated": "TBD"}

        # self._largeImagePath = filePath  # str(self._getLargeImagePath())
        self._largeImagePath = str(self.filePath)
    ...
    def tileSource(self, item, **kwargs):
        ## Note in this case I am using the item.filePath instead of self.filePath
...
# from wsi_deid.process import redact_format_aperio
from wsi_deid.process import (
    aperio_value_list,
...
m = get_standard_redactions(sampleSlide, "ToBeRenamed.svs")
# wsi_deid.process.get_standard_redactions(sampleSlide, "ToBeRenamed.svs")

from wsi_deid.process import get_redact_list, determine_format, fadvise_willneed
...
## Need to monkey patch in an attribute to the TiffFileTileSource
...
def get_large_image_path(self):
    # return the large image path
    print(self)
    print("What properties do I have?")
    return self.filePath


def set_large_image_path(self, value):
    # set the large image path
    print(self)
    self.filePath = value


# Add the property to TiffFileTileSource
TiffFileTileSource._largeImagePath = property(get_large_image_path)
TiffFileTileSource._getLargeImagePath = property(get_large_image_path)

# TiffFileTileSource._largeImagePath = property(
#     get_large_image_path, set_large_image_path
# )
...
def redact_item(item, tempdir):
    """
    Redact a DeIDImageItem based ona girder itemBased on the redact metadata, determine what
    redactions are necessary and perform them.

    :param item: a DeIdImageItem
    ...
    """
    print(type(item))
    # previouslyRedacted = bool(item.get("meta", {}).get("redacted"))  # This is not used
    redactList = get_redact_list(item)
    newTitle = item.filename + "DEID.SVS"
    tileSource = ImageItem().tileSource(item)
    labelImage = None
    ...
    #     labelImage = add_title_to_image(
    #         labelImage, newTitle, previouslyRedacted, item=item
    #     )
    macroImage = None
    # macro_geojson = redactList.get("images", {}).get("macro", {}).get("geojson")
    # redact_square_default = "macro" not in redactList["images"] and config.getConfig(
    #     "redact_macro_square"
    # )
    ...
    print(" I detected the file format is", format)
    func = None
...
redact_item(sampleSlide, "/tmp/helloworld")
